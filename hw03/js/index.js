/**
 * 
 * -------------------------------------
 * DOM Manipulation / Traversal Activity
 * -------------------------------------
 * 
 * 1. Create and attach an event handler (function) to each ".image" 
 * element so that when the ".image" element is clicked, the corresponding 
 * image loads in the .featured image element.
 * 
 * 2. Create event handlers for the next and previous buttons. The next button should
 *    show the next image in the thumbnail list. The previous should show the previous.
 * 
 * 3. If you get to the end, start at the beginning. 
 * 
 * 4. If you get to the beginning, loop around to the end.
 * 
 * 
 */


const images = [
    'images/field1.jpg',
    'images/purple.jpg',
    'images/jar.jpg',
    'images/green.jpg',
    'images/green1.jpg',
    'images/purple1.jpg',
    'images/magnolias.jpg',
    'images/daisy1.jpg'
];

let index = 0;

const initScreen = () => {
    images.forEach((image, idx) => {
        document.querySelector('.cards').innerHTML += `
            <li class="card">
                <div class="image" 
                    style="background-image:url('${image}')"
                    data-index="${idx}"></div>
            </li>`;
    });
};

initScreen();

const changeImage = (ev) => {
    const pic = ev.currentTarget;
    index = parseInt(pic.dataset.index);
    document.querySelector(".featured_image").style.backgroundImage = pic.style.backgroundImage;
}

const nextImage = (env) => {
    if (index>=images.length-1)
    {
        index = 0;
  
    }
    else {
        index +=1;
    }
    console.log(index)
    const next = images[index];
    document.querySelector(".featured_image").style.backgroundImage = `url('${next}')`;

}

const prevImage = (env) => {
    if (index==0)
    {
        index = images.length-1;
  
    }
    else {
        index = index-1;
    }
    console.log(index)
    const next = images[index];
    document.querySelector(".featured_image").style.backgroundImage = `url('${next}')`;

}


//get all image elements
const all_images = document.querySelectorAll('.image');

//attach event handler
for (const img of all_images) {
    img.onclick = changeImage;
}

document.querySelector(".next").onclick = nextImage;
document.querySelector(".prev").onclick = prevImage;